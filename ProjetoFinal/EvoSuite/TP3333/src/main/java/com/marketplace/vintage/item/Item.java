...
public abstract class Item implements Serializable {
    ...
    public <T> T getProperty(ItemProperty property, Class<T> expectedClass) {
        ...
        switch (property) {
            case STOCK:
                result = expectedClass.cast(getCurrentStock());
                break;
            case ITEM_CONDITION:
                result = expectedClass.cast(getItemCondition());
                break;
            case DESCRIPTION:
                result = expectedClass.cast(getDescription());
                break;
            case BRAND:
                result = expectedClass.cast(getBrand());
                break;
            case BASE_PRICE:
                result = expectedClass.cast(getBasePrice());
                break;
            case PARCEL_CARRIER_NAME:
                result = expectedClass.cast(getParcelCarrierName());
                break;
            default:
                throw new IllegalArgumentException("Property not available in item: " + property);
        }
        return result;
    }
