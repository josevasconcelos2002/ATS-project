...
public class OrderFactory {
    ...
    private BigDecimal getSatisfactionPrice(ItemCondition itemCondition) {
        BigDecimal satisfactionPrice;
        switch (itemCondition.getType()) {
            case NEW:
                satisfactionPrice = BigDecimal.valueOf(0.5);
                break;
            case USED:
                satisfactionPrice = BigDecimal.valueOf(0.25);
                break;
            default:
                throw new IllegalArgumentException("Invalid item condition type: " + itemCondition.getType());
        }
