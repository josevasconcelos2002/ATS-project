...
public class MalaItem extends Item {
    ...
    public <T> T getProperty(ItemProperty property, Class<T> expectedClass) {
        if (property == ItemProperty.DIMENSION_AREA) {
            return expectedClass.cast(getDimensionArea());
        } else if (property == ItemProperty.MATERIAL) {
            return expectedClass.cast(getMaterial());
        } else if (property == ItemProperty.COLLECTION_YEAR) {
            return expectedClass.cast(getCollectionYear());
        } else if (property == ItemProperty.DEPRECIATION_RATE_OVER_YEARS) {
            return expectedClass.cast(getDepreciationRateOverYears());
        } else {
