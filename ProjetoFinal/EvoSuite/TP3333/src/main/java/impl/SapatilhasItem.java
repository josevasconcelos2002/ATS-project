...
public class SapatilhasItem extends Item {
    ...
    public BigDecimal getPriceCorrection(int currentYear) {
        ...
        if (itemCondition instanceof UsedItemCondition) {
            UsedItemCondition usedItemCondition = (UsedItemCondition) itemCondition;
    ...
    public <T> T getProperty(ItemProperty property, Class<T> expectedClass) {
        if (property == ItemProperty.SAPATILHA_SIZE) {
            return expectedClass.cast(getSize());
        } else if (property == ItemProperty.HAS_LACES) {
            return expectedClass.cast(hasLaces());
        } else if (property == ItemProperty.COLOR) {
            return expectedClass.cast(getColor());
        } else if (property == ItemProperty.COLLECTION_YEAR) {
            return expectedClass.cast(getCollectionYear());
        } else {
